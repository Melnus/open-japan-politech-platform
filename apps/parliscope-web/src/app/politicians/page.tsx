...
const PARTY_COLORS: Record<string, { gradient: string; border: string; text: string }> = {
  "自由民主党": { gradient: "from-blue-500 to-blue-700", border: "border-blue-500/30", text: "text-blue-400" },
  "自民党": { gradient: "from-blue-500 to-blue-700", border: "border-blue-500/30", text: "text-blue-400" },
  "立憲民主党": { gradient: "from-blue-400 to-cyan-600", border: "border-cyan-500/30", text: "text-cyan-400" },
  "公明党": { gradient: "from-orange-400 to-red-500", border: "border-orange-500/30", text: "text-orange-400" },
  "日本維新の会": { gradient: "from-green-400 to-emerald-600", border: "border-green-500/30", text: "text-green-400" },
  "維新": { gradient: "from-green-400 to-emerald-600", border: "border-green-500/30", text: "text-green-400" },
  "国民民主党": { gradient: "from-yellow-400 to-amber-600", border: "border-yellow-500/30", text: "text-yellow-400" },
  "日本共産党": { gradient: "from-red-500 to-rose-700", border: "border-red-500/30", text: "text-red-400" },
  "共産党": { gradient: "from-red-500 to-rose-700", border: "border-red-500/30", text: "text-red-400" },
  "れいわ新選組": { gradient: "from-pink-400 to-rose-600", border: "border-pink-500/30", text: "text-pink-400" },
  "社民党": { gradient: "from-blue-300 to-indigo-500", border: "border-indigo-500/30", text: "text-indigo-400" },
  "参政党": { gradient: "from-amber-400 to-orange-600", border: "border-amber-500/30", text: "text-amber-400" },
};

const DEFAULT_PARTY_COLOR = { gradient: "from-violet-500 to-indigo-600", border: "border-violet-500/30", text: "text-violet-400" };
  ...
  return (
    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="mb-8 space-y-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
        <div>
          <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-[#6b7280]">議院</span>
        ...
        <div>
          <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-[#6b7280]">政党</span>
        ...
        {politicians.map((pol) => {
          ...
          return (
            <a key={pol.id} href={`/politicians/${pol.id}`} className="group block">
              <div className={`relative overflow-hidden rounded-xl border border-white/[0.06] bg-white/[0.03] p-4 transition-all duration-300 hover:border-white/[0.12] hover:bg-white/[0.05] hover:shadow-lg hover:shadow-indigo-500/5 hover:-translate-y-0.5 ${partyColor.border}`}>
                {/* Top accent line */}
                <div className={`absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r ${partyColor.gradient} opacity-60`} />
                <div className="flex items-center gap-3">
                  <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${partyColor.gradient} text-sm font-bold text-white shadow-lg`}>
                    {pol.name.charAt(0)}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-white group-hover:text-indigo-300 transition-colors">{pol.name}</span>
                      {pol.chamber && (
